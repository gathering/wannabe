...
function fetchSlideshowData()
{
  ...
  docurl = docurl.substring(0, docurl.length-18);
  console.log(docurl);
  var jsondata = $.ajax({
...
function nextSlide(){
  var videos = new Array("/media/bumper_1.720.mp4", "/media/bumper_2.720.mp4");
  ...
  if (currentSlideshow[slideshowIndex].url == null || currentSlideshow[slideshowIndex].url=='scheduleslide') {
   $(div).append('<video position: absolute; top: 0; left: 0;" width="1280" height="720" autoplay="autoplay"><source src="'+videos[randomnumber]+'" type="video/mp4" /></video>');
  }
...
function createFancySlide(slide){
 ...
 function createSlide2011(slide){
  ...
  var titlecontent = document.createElement("div");
  titlecontent.id = "titlecontent";
  titlecontent.style.paddingTop="240px";  
  titlecontent.innerHTML = slide.title;
