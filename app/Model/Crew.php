...
class Crew extends AppModel {
	...
	public function getChildCrews($parent_id, $select=false, $excludeHidden=true) {
		...
		if($crews === false) {
			...
			if($excludeHidden) {
				$crews = $this->query("SELECT * FROM (SELECT * FROM wb4_crews WHERE event_id = '"WB::$event->id"' and hidden=false order by crew_id, id) Crew, (SELECT @pv := '".$parent_id."') initialisation WHERE find_in_set(crew_id, @pv) > 0 AND @pv := concat(@pv, ',', id) ORDER BY sorted_weight, name");
			} else {
				$crews = $this->query("SELECT * FROM (SELECT * FROM wb4_crews WHERE event_id = '"WB::$event->id"' order by crew_id, id) Crew, (SELECT @pv := '".$parent_id."') initialisation WHERE find_in_set(crew_id, @pv) > 0 AND @pv := concat(@pv, ',', id) ORDER BY sorted_weight, name");
			}
	...
	public function getMemberUserRole($crew, $user) {
      ...
      $child_crews = $this->query("SELECT id, crew_id FROM (SELECT id, crew_id FROM wb4_crews WHERE event_id = '"WB::$event->id"' ORDER BY crew_id, id) Crew, (SELECT @pv := '".$current['id']."') initialisation WHERE find_in_set(crew_id, @pv) > 0 AND @pv := concat(@pv, ',', id)");
