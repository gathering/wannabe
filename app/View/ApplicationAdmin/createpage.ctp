<form method="POST"  enctype="multipart/form-data" action="<?=$this->Wb->eventUrl('/ApplicationAdmin/save')?>">
	<fieldset>
		...
		<div class="clearfix">
			...
			<div class="input">
				<?=$this->Form->text('ApplicationPage.name', array('div' => false, 'label' => false))?>
			</div>
		</div>
		<div class="clearfix">
			<label for="data[ApplicationPage][description]"><?=__("Description")?></label>
			<div class="input">
				<?=$this->Form->textarea('ApplicationPage.description', array('div' => false, 'label' => false, 'class' => 'xxlarge', 'rows' => 4))?>
			</div>
		</div>
		<div class="clearfix">
			<label for="data[ApplicationPage][position]"><?=__("Position")?></label>
			<div class="input">
				<?=$this->Form->text('ApplicationPage.position', array('div' => false, 'label' => false))?>
			</div>
		</div>
		<div class="clearfix">
			<label for="data[ApplicationPage][type]"><?=__("Type")?></label>
			<div class="input">
				<?=$this->Form->select('ApplicationPage.type', $types, array('empty' => __("Choose"), 'div' => false, 'label' => false))?>
			</div>
		</div>
	</fieldset>
	<div class="actions">
