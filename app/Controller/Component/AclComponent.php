	...
	public function hasMembershipToCrew($user, $crew_id) {
		foreach ($user['Crew'] as $crew) {
			if ($crew['id'] == $crew_id) {
			    ...
			    $crew_class = New Crew();
                $child_crews = $crew_class->query("SELECT id, crew_id FROM (SELECT id, crew_id FROM wb4_crews WHERE event_id = '"WB::$event->id"' ORDER BY crew_id, id) Crew, (SELECT @pv := '".$crew['id']."') initialisation WHERE find_in_set(crew_id, @pv) > 0 AND @pv := concat(@pv, ',', id)");
